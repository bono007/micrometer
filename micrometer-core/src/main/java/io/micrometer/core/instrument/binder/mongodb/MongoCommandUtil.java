...
class MongoCommandUtil {

    // See https://docs.mongodb.com/manual/reference/command for the command reference
    private static final Set<String> COMMANDS_WITH_COLLECTION_NAME = new LinkedHashSet<>(Arrays.asList(
     ...
     */
    Optional<String> determineCollectionName(final String commandName, final BsonDocument command) {
        if (COMMANDS_WITH_COLLECTION_NAME.contains(commandName)) {
            final Optional<String> collectionName = getNonEmptyBsonString(command.get(commandName));
    ...
    /**
     * @return trimmed string from {@code bsonValue} or empty if value was not a non-empty string
     */
    private Optional<String> getNonEmptyBsonString(final BsonValue bsonValue) {
